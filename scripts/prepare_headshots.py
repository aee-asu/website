...
Each entry names a source, a destination and a focal point. The focal point is
where the face sits in the frame as a fraction of width and height, because a
plain centre crop beheads anyone photographed off-centre or standing tall in a
landscape photo.
...
#: generous enough for a 2x display without carrying a large file.
SIZE = 640

#: src is relative to the repository root. focus is (x, y) as a fraction of the
#: source, marking roughly the centre of the head.
PORTRAITS = [
    {
...
def process(entry: dict) -> None:
    ...
    im = square(ImageOps.exif_transpose(Image.open(src)).convert("RGB"), entry["focus"])
    if im.width > SIZE:
